...
class LWR_Bridge():
    ...
    def open(self):
        ...
        self.qcmded_port.open(
            config.robotarm_portbasename + config.qcmded_portname)
        # yarp port for writing
        self.qcmd_port = yarp.BufferedPortBottle()
        self.qcmd_port.open(
            config.robotarm_portbasename + config.qcmd_portname)
        yconnect = yarp.Network.connect
        cstyle = yarp.ContactStyle()
        cstyle.persistent = True
        robotbn = config.robotarm_portbasename
        if self.sim:
            yconnect(robotbn + config.qcmd_portname,
                     robotbn + "/joint_sim/qvin", cstyle)
        else:
            yconnect(robotbn + config.qcmd_portname, robotbn + "/robot/cmd",
                     cstyle)
        yconnect(robotbn + "/robot/cmded", robotbn + config.qcmded_portname,
                 cstyle)
...
def main():
    ...
    yconnect(robotbn + config.qcmd_portname, robotbn + "/roboview/qvin",
             cstyle)
