...
from arcospyu.yarp_tools.yarp_comm_helpers import (recur,
                                                   yarp_queryname_blocking,
                                                   yarp_connect_blocking)
from arcospyu.dprint import dprint, wprint
            ...
            for objectNum in objects:
                ...
                if objectNum == 0:
                    ...
                    if len(paramlist) == 17:  # normal goal
                        ...
                        parambottleList = parambottle.addList()
                        goalposition = array(
                            [paramlist[3], paramlist[7], paramlist[11]])
                    ...
                    if paramlist[0] == "ObstacleP":  # Point decay repeller
                        ...
                        parambottleList = parambottle.addList()
                        for i in range(3):
                            parambottleList.addDouble(
                                paramlist[i * 4 + 3 + 1])  # position
                        parambottleList.addDouble(
                            paramlist[16 + 1])  # obstacle radius
                        parambottleList.addDouble(0.001)  # safe distance
                        parambottleList.addDouble(
                            paramlist[17 + 1])  # decay order
                        paramPort.writeStrict()
                    if paramlist[0] == "ObstacleH":  # hemisphere repeller
                        ...
                        for i in range(3):
                            parambottleList.addDouble(
                                paramlist[i + 16 + 1])  # normal
                        parambottleList.addDouble(
                            paramlist[19 + 1])  # safe distance
                        parambottleList.addDouble(
                            paramlist[20 + 1])  # decay order
                        paramPort.writeStrict()
        else:
            dprint(
                "Not setting repellers, waiting for a goal. Please set a " +
                "goal, to apply all the repellers"
            )
